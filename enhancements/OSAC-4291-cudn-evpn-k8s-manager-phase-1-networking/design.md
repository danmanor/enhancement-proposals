...
| Subnet's NetworkACL association | Independent ACL scoped to the parent VirtualNetwork; the configured fabric manager reports it active | — | Required before the Subnet is READY |
...
1. **Physical fabric manager** runs when Subnet is created:
   ...
   - Reconciles the independent NetworkACL referenced by the Subnet and reports when its policy is active; one ACL may be reused by multiple Subnets in the VirtualNetwork
   - Extracts VNI and reserved IP ranges
   - Writes output to ConfigMap: `{l2_vni, l3_vni, fabric_reserved_range}`
2. **Subnet controller** waits for fabric provisioning and for the associated NetworkACL policy to be active before reporting the Subnet READY; it passes VNI data from ConfigMap through the existing CUDN provisioning flow
...
sequenceDiagram
    ...
    API->>API: Check whether VMs exist before allowing another Subnet
    Note over API: Reject a second Subnet while VMs exist;<br/>otherwise allow fabric-only Subnets, with VM placement blocked when count > 1
    ...
    Controller-->>Tenant: Subnet Provisioned (status: Ready after policy active)
```
...
func (r *SubnetReconciler) handleDelete(ctx context.Context, subnet *osacv1.Subnet) (reconcile.Result, error) {
    ...
    if hasCUDN {
        ...
        // Then trigger fabric deprovision (fabric manager VNet delete) only after CUDN fully deleted
    } else {
...
- Verify the associated ACL policy is active before Subnet readiness
...
- **Verify set_stats → Job CR status data path:**
  1. Create a VirtualNetwork and a READY, same-VirtualNetwork NetworkACL; create the test Subnet with an explicit ACL reference and fabric_manager="primary"
