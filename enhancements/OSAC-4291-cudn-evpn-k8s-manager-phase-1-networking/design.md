...
2. **Subnet controller** waits for segment provisioning, `NetworkACL.status.phase == "Ready"`, and the Subnet's `NetworkACLAssociationReady=True` condition before reporting the Subnet READY; it passes VNI data from ConfigMap through the existing CUDN provisioning flow
