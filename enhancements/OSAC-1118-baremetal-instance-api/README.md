...
* Integration with OSAC networking resources (`VirtualNetwork`, `Subnet`, `SecurityGroup`) — deferred to a future enhancement. This EP does not add networking fields to `BareMetalInstanceCatalogItem`; resource networking remains owned by the resource provisioning flow. A dedicated networking enhancement will define how tenants create their own `Subnet` and attach it to a `BareMetalInstance`.
